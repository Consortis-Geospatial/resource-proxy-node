...
function getTokenEndpointFromURL(url) {
  ...
  return new Promise(function (resolvePromise, rejectPromise) {
    ...
    QuickLogger.logInfoEvent(
      Configuration.getStringTableEntry('Transform url to token endpoint', {
        url: url,
    ...
    if (
      tokenUrlParts != null &&
      tokenUrlParts.hostname != null &&
      tokenUrlParts.pathname != null &&
      tokenUrlParts.protocol != null
    ) {
      httpRequestPromiseResponse(
        tokenUrlParts.hostname,
        tokenUrlParts.pathname,
        method,
        tokenUrlParts.protocol == 'https',
        parameters
      ).then(
        function (serverResponse) {
          ...
          if (tokenServiceUri != null) {
            ...
            rejectPromise(
              new Error(
                Configuration.getStringTableEntry('Unable to transform to token endpoint', {
                  url: url,
                  tokenUrl: tokenUrl,
                })
              )
      ...
      rejectPromise(
        new Error(
          Configuration.getStringTableEntry('Unable to transform to usable URL', {
            url: url,
            tokenUrl: tokenUrl,
          })
        )
...
function getNewTokenFromUserNamePasswordLogin(referrer, serverUrlInfo) {
  ...
  return new Promise(function (resolvePromise, rejectPromise) {
    if (
      ProjectUtilities.isPropertySet(serverUrlInfo, 'username') &&
      ProjectUtilities.isPropertySet(serverUrlInfo, 'password')
    ) {
      parameters = {
        request: 'getToken',
        f: 'json',
      ...
      getTokenEndpointFromURL(serverUrlInfo.url).then(
        function (tokenServiceUri) {
          ...
          httpRequestPromiseResponse(
            tokenServiceUriParts.host,
            tokenServiceUriParts.path,
            method,
            tokenServiceUriParts.protocol == 'https',
            parameters
          ).then(
...
function performAppLogin(serverURLInfo) {
  ...
  QuickLogger.logInfoEvent(
    Configuration.getStringTableEntry('Service is secured by', {
  ...
  var tokenRequestPromise = new Promise(function (resolvePromise, rejectPromise) {
    ...
    httpRequestPromiseResponse(
      oauthUrlParts.hostname,
      oauthUrlParts.pathname,
      'POST',
      oauthUrlParts.protocol == 'https',
      parameters
    ).then(
      function (serverResponse) {
        ...
        if (tokenResponse.length > 0) {
          ...
          rejectPromise(
            new Error(
              Configuration.getStringTableEntry('App login could not get a token', {
...
function performUserLogin(serverURLInfo, requestUrl) {
  ...
  var tokenRequestPromise = new Promise(function (resolvePromise, rejectPromise) {
    ...
    if (requestUrlParts.pathname.toLowerCase().indexOf('/generatetoken') >= 0) {
      ...
      httpRequestPromiseResponse(
        requestUrlParts.hostname,
        requestUrlParts.pathname,
        'POST',
        requestUrlParts.protocol == 'https',
        parameters
      ).then(
        function (serverResponse) {
          tokenResponse = ProjectUtilities.findTokenInString(serverResponse, 'token');
          if (tokenResponse.length > 0) {
            ...
            rejectPromise(
              new Error(
                Configuration.getStringTableEntry('User login could not get a token', {
                  response: serverResponse,
                })
              )
            );
...
function getNewTokenIfCredentialsAreSpecified(serverURLInfo, requestUrl) {
  return new Promise(function (resolvePromise, rejectPromise) {
    if (serverURLInfo.isAppLogin) {
      ...
      rejectPromise(
        new Error(
          Configuration.getStringTableEntry('No method configured to authenticate', {
...
function exchangePortalTokenForServerToken(portalToken, serverURLInfo) {
  var responsePromise = new Promise(function (resolvePromise, rejectPromise) {
    ...
    httpRequestPromiseResponse(
      ...
      path,
      'POST',
      UrlFlexParser.getBestMatchProtocol('*', oauthUrlParts, serverURLInfo) == 'https',
      parameters
    ).then(
      function (serverResponse) {
        tokenResponse = ProjectUtilities.findTokenInString(serverResponse, 'token');
        if (tokenResponse.length > 0) {
          ...
          rejectPromise(
            new Error(
              Configuration.getStringTableEntry('Could not get a token from server response', {
                response: serverResponse,
              })
            )
          );
...
function processValidatedRequest(urlRequestedParts, serverURLInfo, referrer, request, response) {
  ...
  if (serverURLInfo != null) {
    ...
    if (proxyServer != null) {
      ...
      parameters = UrlFlexParser.combineParameters(
        request,
        urlRequestedParts,
        serverURLInfo,
        serverURLInfo.parameterOverride
      );
      ...
      QuickLogger.logInfoEvent(
        '==> Issuing proxy request [' + request.method + ']' + request.url + ' for ' + proxyRequest
      );
  ...
  try {
    var timeNow = new Date(),
      ...
      responseObject = {
        ...
        'Referrers Allowed': configuration.allowedReferrers
          .map(function (allowedReferrer) {
    ...
    if (rateMeter != null) {
      rateMeter.databaseDump().then(
        function (responseIsArrayOfTableRows) {
    ...
    sendErrorResponse(
      ...
      response,
      500,
      Configuration.getStringTableEntry('System error processing request', {
        message: exception.toLocaleString(),
      })
    );
  }
  QuickLogger.logInfoEvent(
    Configuration.getStringTableEntry('Status request from', {
      referrer: referrer,
    })
  );
}
  ...
  for (key in responseObject) {
    if (responseObject.hasOwnProperty(key)) {
      ...
      if (value instanceof Array) {
        // Arrays get displayed as tables
...
function sendErrorResponse(urlRequested, response, errorCode, errorMessage) {
  ...
  QuickLogger.logErrorEvent(
    Configuration.getStringTableEntry('Request error with info', {
      error: errorMessage,
      code: errorCode,
      url: urlRequested,
    })
...
function checkRateMeterThenProcessValidatedRequest(referrer, requestParts, serverURLInfo, request, response) {
  var statusCode = 200;
  if (rateMeter != null) {
    rateMeter.isUnderRate(referrer, serverURLInfo).then(
      function (isUnderCap) {
        if (isUnderCap) {
          processValidatedRequest(requestParts, serverURLInfo, referrer, request, response);
        } else {
          statusCode = 429; // TODO: or is it 402? or 420?
          QuickLogger.logWarnEvent(
            Configuration.getStringTableEntry('RateMeter blocking access to', {
              url: serverURLInfo.url,
              referrer: referrer,
            })
          );
          sendErrorResponse(
            ...
            response,
            statusCode,
            Configuration.getStringTableEntry('Metered requests exceeded', null)
          );
        }
      },
      function (error) {
        statusCode = 420;
        QuickLogger.logErrorEvent(
          Configuration.getStringTableEntry('RateMeter failed on', {
            url: serverURLInfo.url,
            referrer: referrer,
        ...
        sendErrorResponse(
          ...
          response,
          statusCode,
          Configuration.getStringTableEntry('Metered resource status failed', null)
        );
      }
    );
  } else {
...
function processRequest(request, response) {
  ...
  if (request.url.includes('https:')) {
    requestUrl = !request.url.includes('https://') ? request.url.replace('https:/', 'https://') : request.url;
  } else {
    requestUrl = !request.url.includes('http://') ? request.url.replace('http:/', 'http://') : request.url;
  }
  ...
  if (requestParts != null) {
    ...
    QuickLogger.logInfoEvent(
      Configuration.getStringTableEntry('New request from', {
        referrer: referrer,
    ...
    if (referrer != null) {
        ...
        if (isValidURLRequest(requestParts.listenPath)) {
          ...
          if (serverURLInfo != null) {
            ...
            sendErrorResponse(
              ...
              response,
              404,
              Configuration.getStringTableEntry('Resource not found', {
                url: request.url,
              })
            );
          }
      ...
      sendErrorResponse(
        ...
        response,
        403,
        Configuration.getStringTableEntry('Referrer not allowed', {
          referrer: referrer,
        })
      );
    }
...
function proxyResponseError(error, proxyRequest, proxyResponse, proxyTarget) {
  ...
  QuickLogger.logErrorEvent(
    Configuration.getStringTableEntry('proxyResponseError caught error', {
      code: error.code,
  ...
  sendErrorResponse(
    ...
    Configuration.getStringTableEntry('Proxy request error', {
      code: error.code,
      description: error.description,
    })
  );
...
function proxyErrorHandler(proxyError, proxyRequest, proxyResponse) {
  sendErrorResponse(
    ...
    proxyResponse,
    500,
    Configuration.getStringTableEntry('Proxy error 500', {
...
function startServer() {
  ...
  try {
    ...
    QuickLogger.logInfoEvent(
      'Starting proxy version ' +
        proxyVersion +
        ' running on ' +
        hostName +
        ' via ' +
        (configuration.useHTTPS ? 'HTTPS' : 'HTTP') +
        ' server on port ' +
        configuration.port +
        ' -- ' +
        serverStartTime.toLocaleString()
    );
    ...
    rateMeter = RateMeter(
      ...
      QuickLogger.logErrorEvent.bind(QuickLogger)
    );
    rateMeter.start();
    ...
    if (httpServer != null) {
      ...
      if (waitingToRunIntegrationTests || Configuration.isTestMode()) {
        ...
        if (!Configuration.isTestMode()) {
          QuickLogger.logInfoEvent(
            Configuration.getStringTableEntry('Integration tests complete shutting down from test', null)
          );
          process.exit();
        }
    ...
    QuickLogger.logErrorEvent(
      Configuration.getStringTableEntry('Proxy server startup exception', {
...
function cannotStartServer(reason) {
  QuickLogger.logErrorEvent(
    Configuration.getStringTableEntry('Server not started invalid config', {
...
function cannotListen(reason) {
  QuickLogger.logErrorEvent(
    Configuration.getStringTableEntry('Server not started due to', {
      reason: reason.message,
    })
  );
  process.exit();
...
function exitHandler(options, error) {
  QuickLogger.logEventImmediately(
    QuickLogger.LOGLEVEL.INFO.value,
    Configuration.getStringTableEntry('Stopping server via', {
...
function __runIntegrationTests() {
  var testStr, targetStr, result, serverUrlInfo, urlParts, token;
  ...
  testStr =
    '/proxy/http://route.arcgis.com/arcgis/rest/services/World/ClosestFacility/NAServer/ClosestFacility_World/solveClosestFacility';
  ...
  if (token !== null) {
    ...
    console.log(
      'exchangePortalTokenForServerToken test not run because we do not have a short-lived token to test with'
    );
  }
